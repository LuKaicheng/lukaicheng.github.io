...
其中
- name 表示规则名称，对于它的解释可以参考[1.2 规则名称](#ruleName)，
- elements 可以是一个或者多个规则或者最终值的操作组合，关于最终值可以参考[1.3 最终值](#terminalValue)
- crlf 行结束标志(回车换行)

为了视觉效果，规则定义需要左对齐，如果碰上一个规则需要多行的情况，那么接下来的行需要缩进，而它们对齐和缩进的基准是ABNF规则的第一行。

### <span id="ruleName">1.2 规则名称</span>
规则名称由字母开头，后续可以包含字母、数字和连字符(减号)组合的序列,其中需要注意的是规则名称**不区分大小写**。而且不像BNF，用尖括号(**<**，**>**)包围规则名称并不是必需的。

### <span id="terminalValue">1.3 最终值</span>
